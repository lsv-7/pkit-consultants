...
export default function InvoicePreviewPage({ params }: Props) {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const [resInvoice, resSettings] = await Promise.all([
  ...
  const handleExportPDF = () => {
    window.location.href = `/api/admin/invoices/${id}/pdf`;
  };
  ...
  return (
    <>
      ...
      {/* Screen Toolbar */}
      <div className="no-print max-w-[210mm] mx-auto mb-6 flex justify-between items-center bg-[#111827]/60 border border-[#1E293B] p-4 rounded-xl select-none">
        ...
        <div className="flex gap-2">
          <Button variant="indigo" size="sm" onClick={handleExportPDF} className="gap-1.5 h-9">
            <Download size={14} />
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td className={`meta-value font-bold ${
                        ...
                        invoice.paymentStatus === "PARTIALLY_PAID" ? "status-partially-paid" : "status-pending"
                      }`}>
