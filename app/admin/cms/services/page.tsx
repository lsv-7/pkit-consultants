...
export default function ServicesCMS() {
  ...
  const handleOpenEdit = (s: ServiceItem) => {
    ...
    setFormData({
      ...
      technologiesRaw: s.technologies.join(", "),
    });
    setFormError("");
    setEditorOpen(true);
  };
      ...
      {!editorOpen ? (
                  ...
                  {services.length === 0 ? (
                    ...
                    services.map((s) => (
                      <tr key={s.id} className="hover:bg-[#0C1A3D]/20 transition duration-150">
                        ...
                        <td className="py-4 px-6 font-mono text-xs">{s.icon}</td>
                        <td className="py-4 px-6">
                          <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold ${s.active ? "bg-emerald-500/10 text-emerald-400" : "bg-slate-500/10 text-slate-400"}`}>
        ...
        <div className="space-y-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-slate-100 font-display">{editingId ? "Edit Service" : "New Service"}</h1>
              <p className="text-xs text-slate-400 mt-0.5">Fill out information and capabilities of this service discipline.</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSave} className="space-y-6">
            ...
            <div className="bg-[#060F24] border border-[#0E204A] rounded-xl p-6 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-slate-400">Slug (e.g. ai-solutions)</label>
                  <input
                    type="text"
                    name="slug"
                    value={formData.slug}
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-400">Short Description (for cards / footers)</label>
                <textarea
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-400">Long Description (discipline copy)</label>
                <textarea
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-400">Features / Capabilities (Comma separated)</label>
                <input
                  type="text"
                  name="featuresRaw"
